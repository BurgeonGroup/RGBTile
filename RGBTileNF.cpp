...
uint32_t RGBTileNF::Color(byte r, byte g, byte b)
{
  uint32_t c;
  c = r;
  c <<= 8;
...
byte* RGBTileNF::getLetter(char l)
{
  if(l>31&&l<123)
  {
    return ALNUMS[l-32];
...
void RGBTileNF::drawLetter(char c)
{
//  uint32_t color;
  byte* l = getLetter(c);
  for(int i=0;i<NUM_ROWS;i++)
...
void RGBTileNF::drawPixel(int x, int y)
{
  _strip.setPixelColor(x, y, _colors[x][y]); 
  _strip.show();
}

void RGBTileNF::colorPixel(int x, int y, byte r, byte g, byte b)
{
